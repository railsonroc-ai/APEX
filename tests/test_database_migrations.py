...
def test_privacy_lifecycle_migration_authorizes_only_explicit_student_deletion(
    ...
    try:
        ...
        assert latest["version"] == 21
        assert latest["name"] == "sync_executable_curriculum_v8"
...
def test_v18_database_receives_fifth_curriculum_microconcept(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert dict(row) == {"selectable": 0, "catalog_version": 8}
        assert tuple(latest) == (21, "sync_executable_curriculum_v8")
        assert connection.execute("PRAGMA foreign_key_check").fetchall() == []
        assert connection.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
    finally:
        connection.close()
