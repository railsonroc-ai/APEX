...
def test_v5_database_receives_empty_evidence_ledger(
    ...
    try:
        ...
        assert total == 0
        assert turns == 1
        assert [row["version"] for row in versions] == list(range(1, 23))
        assert connection.execute(
            "PRAGMA foreign_key_check"
        ).fetchall() == []
        assert connection.execute(
...
def test_privacy_lifecycle_migration_authorizes_only_explicit_student_deletion(
    ...
    try:
        ...
        assert latest["version"] == 22
        assert latest["name"] == "sync_executable_curriculum_v9"
...
def test_v18_database_receives_fifth_curriculum_microconcept(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert dict(row) == {"selectable": 0, "catalog_version": 9}
        assert tuple(latest) == (22, "sync_executable_curriculum_v9")
        assert connection.execute("PRAGMA foreign_key_check").fetchall() == []
        assert connection.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
    finally:
        connection.close()
