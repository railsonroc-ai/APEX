...
EXPECTED_MIGRATIONS = [
    ...
    (15, "sync_executable_curriculum_v2"),
    (16, "sync_executable_curriculum_v3"),
    (17, "sync_executable_curriculum_v4"),
    (18, "sync_executable_curriculum_v5"),
    (19, "sync_executable_curriculum_v6"),
    (20, "sync_executable_curriculum_v7"),
    ...
    (25, "sync_executable_curriculum_v12"),
]
...
def test_privacy_lifecycle_migration_authorizes_only_explicit_student_deletion(
    ...
    try:
        ...
        assert latest["version"] == 25
        assert latest["name"] == "sync_executable_curriculum_v12"
...
def test_v14_database_receives_executable_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v14-to-v15.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:14])
    database_module.init_database()
    ...
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:15])
    database_module.init_database()
    connection = connect(path)
    try:
        row = connection.execute(
        ...
        assert dict(row) == {"selectable": 0, "catalog_version": 2}
        assert next_row is None
        assert tuple(latest) == (15, "sync_executable_curriculum_v2")
        assert connection.execute("PRAGMA foreign_key_check").fetchall() == []
    finally:
        connection.close()


def test_v15_database_receives_second_curriculum_microconcept(
    monkeypatch,
    tmp_path,
):
    path = configure_database(monkeypatch, tmp_path, name="v15-to-v16.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:15])
    database_module.init_database()
    ...
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:16])
    database_module.init_database()

    connection = connect(path)
    try:
        row = connection.execute(
            "SELECT selectable, catalog_version FROM concept_definitions "
            "WHERE concept_id = ?",
            ("ads.algorithms.goal_result",),
        ).fetchone()
        latest = connection.execute(
        ...
        assert dict(row) == {"selectable": 0, "catalog_version": 3}
        assert tuple(latest) == (16, "sync_executable_curriculum_v3")
        assert connection.execute("PRAGMA foreign_key_check").fetchall() == []
    finally:
        connection.close()

def test_v16_database_receives_third_curriculum_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v16-to-v17.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:16])
    database_module.init_database()
    ...
    try:
        ...
        assert before is None
        assert goal["catalog_version"] == 3
    finally:
        connection.close()

    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:17])
    database_module.init_database()
    ...
    try:
        ...
        assert dict(row) == {"selectable": 0, "catalog_version": 4}
        assert tuple(latest) == (17, "sync_executable_curriculum_v4")
        assert connection.execute("PRAGMA foreign_key_check").fetchall() == []
        assert connection.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
    finally:
        connection.close()

def test_v17_database_receives_fourth_curriculum_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v17-to-v18.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:17])
    database_module.init_database()
    ...
    try:
        ...
        ipo = connection.execute(
            "SELECT catalog_version FROM concept_definitions WHERE concept_id = ?",
            ("ads.algorithms.input_process_output",),
        ).fetchone()
        assert before is None
        assert ipo["catalog_version"] == 4
    finally:
        connection.close()

    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:18])
    database_module.init_database()
    ...
    try:
        ...
        assert dict(row) == {"selectable": 0, "catalog_version": 5}
        assert tuple(latest) == (18, "sync_executable_curriculum_v5")
        assert connection.execute("PRAGMA foreign_key_check").fetchall() == []
        assert connection.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
    finally:
        connection.close()
...
def test_v18_database_receives_fifth_curriculum_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v18-to-v19.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:18])
    database_module.init_database()
    ...
    try:
        ...
        assert before is None
        assert structured["catalog_version"] == 5
    finally:
        connection.close()
    ...
    try:
        ...
        assert dict(row) == {"selectable": 0, "catalog_version": 12}
        assert tuple(latest) == (25, "sync_executable_curriculum_v12")
        assert connection.execute("PRAGMA foreign_key_check").fetchall() == []
        assert connection.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
    finally:
        connection.close()
...
def test_v19_database_receives_sixth_curriculum_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v19-to-v20.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:19])
    database_module.init_database()
    ...
    try:
        ...
        assert before is None
        assert skeleton["catalog_version"] == 6
    finally:
        connection.close()
...
def test_v20_database_receives_seventh_curriculum_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v20-to-v21.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:20])
    database_module.init_database()
    ...
    try:
        ...
        write_concept = connection.execute(
            "SELECT catalog_version FROM concept_definitions WHERE concept_id = ?",
            ("ads.algorithms.portugol_write",),
        ).fetchone()
        assert before is None
        assert write_concept["catalog_version"] == 7
    finally:
        connection.close()
...
def test_v21_database_receives_eighth_curriculum_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v21-to-v22.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:21])
    database_module.init_database()
    ...
    try:
        ...
        read_concept = connection.execute(
            "SELECT catalog_version FROM concept_definitions WHERE concept_id = ?",
            ("ads.algorithms.portugol_read",),
        ).fetchone()
        assert before is None
        assert read_concept["catalog_version"] == 8
    finally:
        connection.close()
...
def test_v22_database_receives_ninth_curriculum_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v22-to-v23.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:22])
    database_module.init_database()
    ...
    try:
        ...
        variable_concept = connection.execute(
            "SELECT catalog_version FROM concept_definitions WHERE concept_id = ?",
            ("ads.algorithms.variable_storage",),
        ).fetchone()
        assert before is None
        assert variable_concept["catalog_version"] == 9
    finally:
        connection.close()
...
def test_v24_database_receives_eleventh_curriculum_microconcept(monkeypatch, tmp_path):
    path = configure_database(monkeypatch, tmp_path, name="v24-to-v25.db")
    current_migrations = migrations_module.MIGRATIONS
    monkeypatch.setattr(migrations_module, "MIGRATIONS", current_migrations[:24])
    database_module.init_database()
    ...
    try:
        ...
        assert before is None
        assert read_concept["catalog_version"] == 11
    finally:
        connection.close()
