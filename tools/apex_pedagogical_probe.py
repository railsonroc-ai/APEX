...
def classify_task(prompt: str | None) -> str | None:
    ...
    if all(marker in folded for marker in ("variavel tentativas: inteiro", "complete somente a lacuna", "nome que falta")):
        return "read_variable_complete_attempts"
    if all(marker in folded for marker in ("variavel pontos: inteiro", "somente com o comando", "recebe uma entrada nessa variavel")):
        return "read_variable_call_points"
    if all(marker in folded for marker in ("coloque em ordem estes elementos", "leia(idade)", "idade: inteiro", "fimalgoritmo", "var", 'algoritmo "cadastro"')):
        return "read_variable_program_order"
    if all(marker in folded for marker in ("de memoria", "variavel saldo: inteiro", "somente com o comando", "recebe uma entrada nessa variavel")):
        return "read_variable_review_balance"
    ...
    if all(marker in folded for marker in ("observe leia", "sem preencher a lacuna", "entrada ou saida")):
        return "read_role_input"
    if all(marker in folded for marker in ("complete somente a lacuna", "inicio", "escreva", "fimalgoritmo", "recebe uma entrada")):
        return "read_before_write"
    if all(marker in folded for marker in ("represente apenas a ordem", "algoritmo", "inicio", "leia", "escreva", "fimalgoritmo", "nao complete o interior")):
        return "read_flow_integration"
    if all(marker in folded for marker in ("de memoria", "qual comando do portugol", "recebe uma entrada")):
        return "read_review_role"
...
def correct_answer(kind: str) -> str:
    answers = {
        ...
        "read_keyword_input": "leia",
        "read_role_input": "entrada",
        ...
        "read_variable_identify_age": "idade",
        "read_variable_complete_attempts": "tentativas",
...
def run_happy_path(report: ProbeReport, timeout: float):
    ...
    with Sidecar(ROOT, timeout=timeout) as sidecar:
        ...
        read_tasks = [kind for kind in seen_kinds if kind.startswith("read_")]
        report.check(
            f"{scenario_id}.portugol_read_progression_reached",
            {
                "read_keyword_input",
                "read_role_input",
                "read_before_write",
                "read_flow_integration",
            }.issubset(set(read_tasks)),
        ...
        read_variable_tasks = [kind for kind in seen_kinds if kind.startswith("read_variable_")]
        report.check(
            f"{scenario_id}.read_variable_progression_reached",
            {
                "read_variable_identify_age",
                "read_variable_complete_attempts",
                "read_variable_call_points",
                "read_variable_program_order",
            }.issubset(set(read_variable_tasks)),
...
def structural_checks(report: ProbeReport):
    ...
    has_next_after_portugol_write = bool(
        re.search(
            r"PORTUGOL_WRITE\s*:\s*PORTUGOL_READ",
            curriculum,
        )
    )
    if has_next_after_portugol_write:
        report.check(
            "structural.next_after_portugol_write",
            True,
            "Curriculum define entrada simples com leia como sucessor executável após escreva.",
        )
    else:
        report.warn(
            "structural.next_after_portugol_write",
    ...
    has_next_after_integer_declaration = bool(
        re.search(
            r"INTEGER_DECLARATION\s*:\s*READ_VARIABLE",
            curriculum,
        )
    )
    if has_next_after_integer_declaration:
        report.check(
            "structural.next_after_integer_declaration",
            True,
            "Curriculum define entrada em variável com leia como sucessor executável após declaração inteira.",
        )
    else:
        report.warn(
            "structural.next_after_integer_declaration",
